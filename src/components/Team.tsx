    ...
    <section id="team" className="relative py-14 sm:py-20 dfa-glow-soft">
      <div className="mx-auto max-w-5xl px-5">
        <div className="text-center">
          ...
          <p className="mt-4 mx-auto max-w-2xl text-slate-500">{site.team.blurb}</p>
        </div>

        <div className="mt-12 grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {site.team.roles.map((r) => (
            <div key={r.title} className="dfa-card rounded-2xl p-6 text-center">
              <div className="mx-auto grid place-items-center h-12 w-12 rounded-full bg-gradient-to-br from-[#ff2d2d] to-[#ff5e3a] text-white font-bold">
                {r.title.charAt(0)}
              </div>
              <h3 className="mt-4 font-semibold text-slate-900 text-sm">{r.title}</h3>
